...
import pandas as pd
from input_files import load_upload_set
from app import UPLOAD_DIR, INPUT_MANIFEST

input_files = load_upload_set(UPLOAD_DIR, INPUT_MANIFEST)
target_bc = "KPBP18-5050-GRI"
...
for key, p in input_files.items():
    ...
    try:
        ...
        if bcol:
            ...
            if not found.empty:
                print(f"\n[+] Found in key '{key}' (File: {Path(p).name}):")
...
# Check all excel files in UPLOAD_DIR recursively
